...
ISR(USART_TX_vect) { uartTxBusy = 1; }
...
uint16_t uartReadCount(void) { return rxCount; }
...
// Start sending the data through the i2c bus
void i2cSendStart() {
...
// Send the signal to stop i2c
void i2cSendStop() { TWCR = (1 << TWINT) | (1 << TWEN) | (1 << TWSTO); }

// Send the information over the i2c address
void i2cSendAddress() {
  TWDR = ADDRESS;
  TWCR = (1 << TWINT) | (1 << TWEN);
  while (!(TWCR & (1 << TWINT))) {
...
// Mother function to send information over i2c
